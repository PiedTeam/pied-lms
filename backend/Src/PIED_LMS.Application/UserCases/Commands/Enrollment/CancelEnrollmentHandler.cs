...
{
    public async Task<ServiceResponse<string>> Handle(Command.CancelEnrollmentCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (enrollment is null)
            return new ServiceResponse<string>(false, "Enrollment not found or unauthorized.", null, null, true,
                "ENROLLMENT_NOT_FOUND");

        if (enrollment.Status == EnrollmentStatus.Cancelled)
            return new ServiceResponse<string>(false, "Enrollment is already cancelled.", null, null, false,
                "ALREADY_CANCELLED");

        if (enrollment.Status == EnrollmentStatus.Rejected)
            return new ServiceResponse<string>(false, "Enrollment has already been rejected.", null, null, false,
                "ALREADY_REJECTED");
        ...
        if (timeUntilStart.TotalHours < 48)
            return new ServiceResponse<string>(false, "Cannot cancel enrollment. Course starts in less than 48 hours.",
                null, null, false, "CANCELLATION_POLICY_VIOLATED");
