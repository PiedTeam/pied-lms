...
{
    public async Task<ServiceResponse<Guid>> Handle(Command.EnrollCourseCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (course.Status != CourseStatus.Active)
            return new ServiceResponse<Guid>(false, "Course is not active.", Guid.Empty, null, false,
                "COURSE_NOT_ACTIVE");

        // Check if already enrolled or pending
        var existingEnrollment = await unitOfWork.Repository<Domain.Entities.Enrollment>()
        ...
        if (existingEnrollment is not null)
            return new ServiceResponse<Guid>(false,
                "You are already enrolled or have a pending request for this course.", Guid.Empty, null, false,
                "ALREADY_ENROLLED");
        ...
        if (course.PrerequisiteCourses.Count != 0)
        {
            ...
            var completedPrerequisites = await unitOfWork.Repository<Domain.Entities.Enrollment>()
                .FindAll(e =>
                    e.UserId == studentId && prerequisiteIds.Contains(e.CourseId) &&
                    e.Status == EnrollmentStatus.Completed)
                .Select(e => e.CourseId)
            ...
            {
                var missingTitles =
                    missingPrerequisiteIds.Select(id => prerequisiteTitles.GetValueOrDefault(id, id.ToString()));
                return new ServiceResponse<Guid>(false,
                    $"Bạn chưa đủ điều kiện. Cần hoàn thành các khóa sau: {string.Join(", ", missingTitles)}",
                    Guid.Empty, null, false, "MISSING_PREREQUISITES");
            }
        }

        // Upload payment proof
        var paymentProofKey = string.Empty;
        if (request.PaymentProof is not null && request.PaymentProof.Length > 0)
            ...
            {
                logger.LogError(ex, "Failed to upload payment proof for user {UserId} to course {CourseId}", studentId,
                    request.CourseId);
                return new ServiceResponse<Guid>(false, "Failed to upload payment proof: " + ex.Message, Guid.Empty,
                    null, false, "UPLOAD_FAILED");
            }
        else
            return new ServiceResponse<Guid>(false, "Payment proof is required.", Guid.Empty, null, false,
        ...
        await unitOfWork.Repository<Domain.Entities.Enrollment>().AddAsync(enrollment, cancellationToken);
        await unitOfWork.Repository<EnrollmentHistory>().AddAsync(history, cancellationToken);

        await unitOfWork.CommitAsync(cancellationToken);

        return new ServiceResponse<Guid>(true, "Enrollment requested successfully. Please wait for admin approval.",
            enrollment.Id);
    }
}
