...
{
    public async Task<ServiceResponse<string>> Handle(UpdateQuestionQuizCommand request, CancellationToken cancellationToken)
    {
        // 1. Auth Check
    ...
    private Domain.Constants.QuestionType ParseQuestionType(string typeStr)
    {
        ...
        if (typeStr.Equals("Multiple choice", StringComparison.OrdinalIgnoreCase)) return PIED_LMS.Domain.Constants.QuestionType.MultipleChoice;
        if (typeStr.Equals("Single choice", StringComparison.OrdinalIgnoreCase)) return PIED_LMS.Domain.Constants.QuestionType.SingleChoice;

        return PIED_LMS.Domain.Constants.QuestionType.MultipleChoice; // Default
    }
}
