...
public class TestCaseStorageService : ITestCaseStorageService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!Directory.Exists(testCaseDir)) Directory.CreateDirectory(testCaseDir);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                await Task.Run(() => Directory.Delete(testCaseDir, true), cancellationToken);
    ...
    public async Task<IReadOnlyList<TestCase>> LoadTestCasesForExamAsync(
        Guid examId,
        CancellationToken cancellationToken = default)
    {
        try
    ...
    private string GetTestCaseDirectory(Guid examId, int index) =>
        Path.Combine(_storageBasePath, examId.ToString(), $"tc_{index}");
