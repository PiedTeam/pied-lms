...
dotnet watch run
```
Backend sẽ khởi chạy tại: `http://localhost:5000` (hoặc port bạn đã cấu hình trong `appsettings.json`).
Swagger UI thường ở: `http://localhost:5000/swagger`
...
- **Migrations:** Nếu DB chưa có table, chạy lệnh sau ở thư mục `backend`:
  ...
  ```
  *(Cần cài đặt `dotnet-ef` tool: `dotnet tool install --global dotnet-ef`)*
